...
class CurrencyConverterApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(self, text='RUB').grid(
            row=0, column=2, sticky='w', padx=5, pady=5
        )

        ttk.Label(self, text='Срок кредита (в месяцах)').grid(
            row=1, column=0, sticky='w', padx=5, pady=5
        )
        ttk.Entry(self, textvariable=self.loan_time_var).grid(
            row=1, column=1, sticky='we', padx=5, pady=5
        )
        ttk.Label(self, text='Мес.').grid(
        ...
        ttk.Button(
            self, text='Рассчитать кредит', command=self.calculate_loan
        ).grid(row=3, column=1, columnspan=1, pady=10, sticky='we')

        self.monthly_label = ttk.Label(
            self, text='Ежемесячный платёж: 0 RUB'
        )
        self.monthly_label.grid(
            row=4, column=0, columnspan=3, sticky='n', padx=5
        )

        self.loan_sum_label = ttk.Label(
            self, text='Сумма всех платежей: 0 RUB'
        )
        self.loan_sum_label.grid(
            row=5, column=0, columnspan=3, sticky='n', padx=5
        )

        self.interest_label = ttk.Label(
            self, text='Начисленные проценты: 0 RUB'
        )
        self.interest_label.grid(
            row=6, column=0, columnspan=3, sticky='n', padx=5
        )

        ttk.Label(self, text='Исходная валюта:').grid(
        ...
        ttk.Label(self, text='Выберите валюту:').grid(
        ...
        ttk.Button(
            self, text='Конвертировать', command=self.convert
        ).grid(row=9, column=1, columnspan=1, pady=10, sticky='we')
        ...
        ttk.Button(
            self, text='Обновить курс валют', command=self.update_db
        ).grid(row=11, column=1, columnspan=1, pady=10, sticky='we')

        self.log_text = tk.Text(
            self, height=6, width=70, wrap="word", state='disabled'
        )
        self.log_text.grid(
            row=12, column=0, columnspan=3, padx=5, pady=5, sticky='nsew'
        )
    ...
    def calculate_loan(self):
        ...
        self.monthly_label.config(
            text=f'Ежемесячный платёж: {monthly_payment:.2f} RUB'
        )
        self.loan_sum_label.config(
            text=f'Сумма всех платежей: {total_payment:.2f} RUB'
        )
        self.interest_label.config(
            text=f'Начисленные проценты: {interest_amount:.2f} RUB'
        )
    ...
    def convert(self):
        ...
        self.result_label.config(
            text=f'Результат конвертации: {result:.2f} {target}'
        )
